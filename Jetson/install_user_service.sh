...
ExecStart=$PYTHON_BIN $BASE_DIR/derived_insight_server.py --token-file $TOKEN_FILE --output $OUTPUT_FILE --database $DATABASE_FILE --adaptive-config $ADAPTIVE_CONFIG_FILE
Restart=always
RestartSec=5
TimeoutStopSec=15
KillSignal=SIGINT
...
systemctl --user enable --now onmom-derived-insights.service
